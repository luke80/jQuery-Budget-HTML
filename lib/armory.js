...
function initializeAds(theLang)
{
	...
	{	
		...
		if(isNotHomePage == "true"){
			//top, long ad
			$("#ad_728x90").html('<iframe width=728 height=90 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling="no" bordercolor="#000000" src="http://gamead.smartad.co.kr/NMR?VNHTML&MN3&SN3&PN6&TN10&DNgamead.smartad.co.kr"></iframe>');	
			//middle box ad
			$("#ad_300x250").html('<iframe width=300 height=250 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling="no" bordercolor="#000000" src="http://gamead.smartad.co.kr/NMR?VNHTML&MN3&SN3&PN6&TN9&DNgamead.smartad.co.kr"></iframe>');	
		} else {
			//middle box ad
			$("#ad_300x250").html('<iframe width=300 height=250 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling="no" bordercolor="#000000" src="http://gamead.smartad.co.kr/NMR?VNHTML&MN3&SN3&PN5&TN8&DNgamead.smartad.co.kr"></iframe>');			
		}
...
function ajaxChangePinned(nameOne, realmOne, allChars){

	var curPage = '/vault/character-select-submit.xml?';
	var charArray = new Array();
	var charCounter = 2;

	var queryString = "n1="+ encodeURI(nameOne) + "&r1="+ encodeURI(realmOne);

	charArray = allChars.split(",");
	for (i = 0; i < charArray.length; i++) {
		charArray[i] = charArray[i].split("_");
		if (nameOne != charArray[i][0] || realmOne != charArray[i][1]) {
			queryString += "&n"+ charCounter +"="+ encodeURI(charArray[i][0]) + "&r"+ charCounter +"="+ encodeURI(charArray[i][1]);
			charCounter++;
		}
	}
...
function showTip(toolTipContent,defaultWidthOverride) {
	...
	if(Browser.ie || Browser.safari || Browser.opera) elemttc.style.width = "300px";
...
function adjustTooltipPosition() {
	...
	else
	{
		...
		var ttipOvershoot = (!Browser.safari) ? elemttc.offsetHeight + mouseY + 10 : elemttc.offsetHeight + mouseY + 10 - varYoffset;
		var ttipSideOvershoot = elemttc.offsetWidth + mouseX + 10;
		if(ttipOvershoot > windowHeight) {
			elemttc.style.top = (!Browser.safari) ? windowHeight - elemttc.offsetHeight + elemDoc.scrollTop + 2 + "px" : windowHeight - elemttc.offsetHeight + varYoffset + "px";
		} else {
...
function showTipTwo(toolTipContent) {
	if(getcookie2("armory.cookieDualTooltip") != 0){
...
//////////////////////////////////////////////////////////////////////////////////
////// Updates tooltip location on mousemove
//////////////////////////////////////////////////////////////////////////////////
...
function replaceLink(siteUrl){
    ...
    document.getElementById("reportLink").href=document.getElementById("reportLink").href.replace("REPLACEURL",replaceUrl.replace('&','%26')).replace("REPLACEBROWSER",browserVer)
}
...
function forumLink(forumName, lang){
    ...
    if(region=="EU")
        ...
        forumsUrl="http://forum.wowtaiwan.com.tw/";
...
function menuCheckLength(formReference){
	...
	if(IS_ENABLED_XSLT){
		...
		if(Browser.ie)//escape the entry
		...
		window.location.href = "/search.xml?searchQuery="+theSearch+"&searchType="+formReference.searchType.value;
...
function getArenaIconBorder(teamSize, whichBadge) {
	...
	$("#"+whichBadge).attr("src","/images/badge-border-pvp-"+ theIcon +".gif");
	$("#"+theIconDivId).css("background-image","url('/images/icons/badges/arena/"+ theIconImage +".jpg')");
}
...
{
	//go through each link with class "thisTip"
	$("a.thisTip").each(function(){
		//make sure there is an upgrade for that item
		if(!$(this).parent().hasClass("noUpgrade") && (this.id != "")){
			$(this).after("<b class=\"hitBoxArrow\"><div> </div></b>"+
				"<div style=\"display: none;\" class=\"fly-horz\">"+
				"<a class=\"upgrd\" href=\"/search.xml?searchType=items&"+recUpgradeURL+"&pi="+this.id+"\">"+theUpgradeTxt+"</a></div>");
		}
	});
	...
	//**************************************************************
	//items on the left
	$(".items-left .hitBoxArrow").hover(
		function(){
			//mouseover actions
			$(this).next().css("display","block");
			$(this).prev().attr("style","background-position: -1px 0");
		},
		function(){
			//mouseout actions
	...
	//items on the right
	$(".items-right .hitBoxArrow").hover(
		function(){
			//mouseover actions
			$(this).next().css("display","block");
			$(this).prev().attr("style","background-position: -74px 0");
		},
		function(){
			//mouseout actions
			$(this).next().css("display","none");
	...
	//bottom items
	$(".items-bot .hitBoxArrow").hover(
		function(){
			//mouseover actions
			$(this).next().css("display","block");
			$(this).prev().attr("style","background-position: 0 -74px");
		},
		function(){
			//mouseout actions
			$(this).next().css("display","none");
...
function bindLoginLink()
{
	...
	if(currLink.indexOf("?") == -1){
		...
		$("#loginLinkRedirect2").attr("href", currLink + "?login=1");
	}else{
		$("#loginLinkRedirect").attr("href", currLink + "&login=1");
		$("#loginLinkRedirect2").attr("href", currLink + "&login=1");
	}
